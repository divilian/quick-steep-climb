...
% TODO: nifty words: eigenvalue eigenvector eigendecomposition eigenbasis
...
And it maps {\footnotesize $\begin{bmatrix} -3 \\ 5 \end{bmatrix}$} to:
...
$\begin{bmatrix} -2 \\ 1 \end{bmatrix}$} is sort of magic: if we feed it as
input to $M$, the output is \textit{a scaled version of the same vector}. In
fact, it exactly doubled in size.

\smallskip

Let's try that again, with the {\footnotesize $\begin{bmatrix} -4 \\ 2
\end{bmatrix}$} we just got:
