...
tell you that its coordinates are $2$ and $-1$, but before you try to visualize
...
the standard basis. If I said a vector had components ``3'' and ``4,'' it has
gone without saying that I meant ``3 units in the $[\ 1\ \ 0\ ]$ direction, and
4 units in the $[\ 0\ \ 1\ ]$ direction.'' Viewed in this way, a vector with
coordinates 3 and 4 is really a linear combination of the two (standard) basis
vectors:
...
\vspace{-.25in}
\begin{align*}
\begin{bmatrix} 3 \\ 4 \\ \end{bmatrix}_{B_s}.
\end{align*}
\vspace{-.15in}
...
\vspace{-.15in}
\begin{align*}
\overrightarrow{r} = \begin{bmatrix} 2 \\ -1 \\ \end{bmatrix}_{B_d}.
\end{align*}
\vspace{-.15in}
...
his coordinates are ``2'' and ``$-1$,'' he merely means ``I've got two
...
\begin{bmatrix} -2 \\ 0 \end{bmatrix}_{B_s}.
\end{align*}
\vspace{-.15in}
...
\vspace{-.15in}
\begin{align*}
\begin{bmatrix} 2 \\ -1 \end{bmatrix}_{B_d} =
\begin{bmatrix} -2 \\ 0 \end{bmatrix}_{B_s}.
\end{align*}
\vspace{-.15in}

You can see in Figure~\ref{basis} that whether we say ``two 
...
zero {\scriptsize $\begin{bmatrix} 0 \\ 1 \end{bmatrix}$},'' we get the same
vector $\overrightarrow{\textbf{r}}$.

\begin{figure}[ht]
\centering
\includegraphics[width=0.6\textwidth]{basis.png}
\caption{In either the domino basis or the standard basis, Ron's vector is the
same.}
\label{basis}
\end{figure}
...
What are her coordinates in the domino basis?
...
we haven't covered yet. I promise we'll return to it, though!
