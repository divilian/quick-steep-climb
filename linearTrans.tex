...
\begin{figure}[ht]
\centering
\includegraphics[width=0.8\textwidth]{notSurjective.pdf}
...
\begin{figure}[hb]
\centering
\includegraphics[width=0.8\textwidth]{notInjective.pdf}
...
\begin{figure}[hb]
\centering
\includegraphics[width=0.8\textwidth]{bijective.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.8\textwidth]{singular.pdf}
...
In this appendix, we'll write a function that will visually display the
behavior of a 2-dimensional linear operator, so we can get our head around what
each one does to the points it takes as input.
...
\index{pylab.arrow@\texttt{pylab.arrow()}}
...
Put this all together, and we can use this code to create
...
Okay. Now let's write a function which will take a $2\times 2$ matrix as an
argument, generate some random inputs, and plot an arrow (like the one above)
for each input showing which point it gets mapped to. The complete code is in
Figure~\ref{fig:plotop}.

The meat of this function is actually all on one line: \texttt{output\_vector =
M.dot(input\_vector)}. That's what takes each randomly-generated input vector
and performs matrix-vector multiplication on it to yield the corresponding
output. Everything else is random number generation and window dressing.

\begin{figure}[ht]
\index{plot\_operator@\texttt{plot\_operator()}}
\centering
\begin{Verbatim}[fontsize=\small,samepage=true,frame=single,framesep=3mm]
def plot_operator(M):
    for i in arange(100):
        xval = random.rand() * 10 - 5
        yval = random.rand() * 10 - 5
        input_vector = array([xval, yval])
        pylab.plot(xval, yval, 'o')

        output_vector = M.dot(input_vector)

        pylab.arrow(input_vector[0], input_vector[1],
    ...
    pylab.show()
\end{Verbatim}
\caption{The \texttt{plot\_operator()} function.}
\label{fig:plotop}
\end{figure}

Let's take it for a spin with a few matrices:

\begin{Verbatim}[fontsize=\small,samepage=true,frame=single,framesep=3mm]
stretch = array([[2,0],[0,2]])
plot_operator(stretch)

flip_horiz_shrink_vert = array([[-1,0],[0,.8]])
plot_operator(flip_horiz_shrink_vert)

rotate_90_ccw = array([[0,1],[-1,0]])
plot_operator(rotate_90_cw)

rad_17 = 17 * pi / 180
rotate_17deg_ccw = array([[cos(rad_17),-sin(rad_17)],
    [sin(rad_17),cos(rad_17)]])
plot_operator(rotate_17deg_ccw)
\end{Verbatim}

\pagebreak

For that last one, we had to first convert 17\textdegree to radians (by
multiplying by $\frac{\pi}{180}$) and then plug that value into the sines and
cosines of the general rotation matrix on p.~\pageref{rotationMatrix}. The
results of these four operators are shown in the plots in Figure~\ref{opPlots}.

\begin{figure}[ht]
\centering
\includegraphics[width=0.48\textwidth]{stretch.png}
\includegraphics[width=0.48\textwidth]{flip_horiz_shrink_vert.png} \\
\includegraphics[width=0.48\textwidth]{rotate_90deg_cw.png}
\includegraphics[width=0.48\textwidth]{rotate_17deg_ccw.png} \\
\caption{The operations of four matrices depicted graphically on random input
vectors: \texttt{stretch} (upper-left), \texttt{flip\_horiz\_shrink\_vert}
(upper-right), \texttt{rotate\_90deg\_ccw} (lower-left), and
\texttt{rotate\_17deg\_cw} (lower-right).}
\label{opPlots}
\end{figure}
