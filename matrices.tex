...
for all the rows $i$ and columns $j$ of the $A$ matrix.
...
Surprise number one is that we multiplied a $2\times 3$ matrix by a column
vector and got back a $2\times 1$ matrix. Ooookay. Surprise number two are the
...
here was \textit{compute \underline{each} row of the matrix dot-product-ed with
the vector}. A dot product, you'll recall, gives us a single scalar as an
answer. So, since $[\ 2\ \ 2\ \ 7\ ] \cdot [\ 1\ \ -1\ \ 4\ ]$ is 28, and $[\
1\ \ 4\ \ 0\ ] \cdot [\ 1\ \ -1\ \ 4\ ]$ is $-3$, our result is a vector
containing those two results.
...
as the dimension of (or the number of entries in) the vector. If these do not
match, the game is over.
\item If 1 and 2 check out, then the operation is permissible, and the result
is a column vector whose dimension (number of entries) is the same as the
number of \textit{rows} (not columns) in the matrix.
\end{compactenum}
...
\item A tall $18\times 3$ matrix times a 3-dimensional column vector is legal,
and gives an 18-dimensional vector result.
\item A wide $5\times 17$ matrix times a 17-dimensional \textit{row} vector is
illegal.
\item A wide $5\times 17$ matrix times a \textit{5}-dimensional column vector is
illegal.
\end{compactitem}
...
it, but it's surprisingly difficult to move across the page with your left
eyeball at the same time you're moving \textit{down} the page with your right
eyeball. Like all things, practice.
