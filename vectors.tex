...
direction and a magnitude, but not a starting point}. (The word
``\textbf{magnitude}'' simply means how long\footnote{We have to be careful
with our terminology here, because we'll use the word \textbf{length} to
describe the number of elements of a vector, which of course is totally
different than the magnitude as drawn in space. The vector in the present
example has a magnitude in the two hundreds, but a length of only 2.} the
vector is from its tail to its tip.) In the case of Stephen's biometrics, the
vector's direction is east-by-northeast-ish (about 20\textdegree\
counter-clockwise from the $x$-axis) and its magnitude is 220 or so, but it
doesn't have any intrinsic ``point of origin''; it's just an arrow pointing
this-a-way and yea-far, no matter where it might be anchored.
...
\index{tail (of a geometric vector)}
\index{tip (of a geometric vector)}

Geometrically, we can compute a vector's direction and magnitude using
principles from trigonometry. Figure~\ref{fig:directionMag} shows a vector
$\overrightarrow{\textbf{v}} = [\ 74 \ 210\ ]$ pictorially. Its $0^\text{th}$
coordinate (a.k.a. $v_0$) is 74, measured on the $x$-axis, and $v_1$ is 210.
...
\caption[.]{The magnitude ($r$) and direction ($\theta$) of the vector
$\overrightarrow{\textbf{v}} = [\ 74 \ 210\ ]$. The direction $\theta$ is the
...
In this case, $r$ works out to 222.6 and $\theta$ is 70.6\textdegree. Think
...
\textbf{Cartesian coordinates}. They're written this way: $222.6 \measuredangle
70.6$\textdegree.
...
step 4 of the hierarchy (Figure~\ref{fig:fiveWays}), in which there's not a
number in sight. This example holds the favorite colors for each of our four
friends, which are of course non-numeric.
...
really the same as key/value-pair-based associative arrays (called
``dictionaries'' or ``hash tables'' in some programming languages) with an
index number as the key; and that both were equivalent to functions (with
inputs being keys/indices and outputs being values) as well. Wow. Sometimes it
seems like the universe is really all just one thing.
