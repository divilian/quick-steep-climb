...
type from the NumPy library.\footnote{An ``array'' is basically another name
for a list of items; Python, in fact, also has a simpler type called a
\textbf{list} that could be used as a primitive, makeshift sort of vector.
NumPy arrays are the better choice for data-centric applications. Also, the
first two letters of \texttt{ndarray} stand for ``$n$-dimensional.''} Creating
an \texttt{ndarray} (or just ``array'' for short) has the effect of introducing
a named variable whose value is a whole sequence of numbers, not just a single
number like a scalar variable has. In Python, each element of an array has an
consecutive index number, which as I mentioned on
p.~\pageref{PythonStartsAtZero}, start at 0 instead of at 1.

In our first example, we use the \texttt{array()} NumPy function to create a
history of local temperatures:
...
(I'm writing this chapter in the wintertime, by the way, which may explain my
frosty Fredericksburg.)

We now have a new array that goes by the name \texttt{temps}. We can use the
\texttt{print()} function to see all its values, and we can include numbered
indices (indexes) inside a pair of ``boxies'' (square brackets) after the array
name to look at individual items.
...
Take notice! In a five-element array, since element \#0 is the first one,
element \#4 is the last one.
...
elements: the ones at index numbers 1, 2, and 3. The fact that 4 is
\textit{not} included in the slice \texttt{1:4} is yet another minor but weird
thing you have to get used to. (Maybe you should make a list of these.)
